...
import { MAppBar, MTabs, MTab, MBox } from 'src/app/material-ui';
...
export default function TabsHorizontal(props: any) {
    ...
    useEffect(() => {
        ...
        // console.log(props.selectedCompnent);
        setValue(tabIndex);
    }, [window.location.hash]);

    return (
        <div className={classes.tabsWrapper}>
            <MAppBar position="static" className={props.istrue ? `${classes.tabs}` : `${classes.tabsBar}`}>
                <MTabs
                    ...
                    {props.data.map((content: any, index: number) => (
                        ...
                        <NavLink className={classes.tabNavLink} to={'#' + content.hash}
                            onClick={() => props.tabClick && props.tabClick(new Date())}>
            ...
            <MBox my={3} className={props.istrue ? `${classes.panelWr}` : `${classes.panelWrapper}`} borderRadius={4}>
                {props.data.map((content: any, index: number) => (
